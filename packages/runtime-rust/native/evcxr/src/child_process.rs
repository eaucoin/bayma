...
pub(crate) struct ChildProcess {
    ...
    stderr_barrier_sender: crossbeam_channel::Sender<String>,
    stderr_barrier_receiver: crossbeam_channel::Receiver<String>,
    expected_stderr_barrier: Arc<Mutex<Option<String>>>,
...
impl ChildProcess {
    pub(crate) fn new(
        ...
        let (stderr_barrier_sender, stderr_barrier_receiver) = crossbeam_channel::unbounded();
        let expected_stderr_barrier = Arc::new(Mutex::new(None));
        ChildProcess::new_internal(
            Arc::new(Mutex::new(command)),
            None,
            Arc::new(Mutex::new(stderr_sender)),
            stderr_barrier_sender,
            stderr_barrier_receiver,
            expected_stderr_barrier,
    ...
    fn new_internal(
        ...
        std::thread::spawn({
            ...
            let stderr_barrier_sender = stderr_barrier_sender.clone();
            let expected_stderr_barrier = Arc::clone(&expected_stderr_barrier);
            let pending_output_lines = Arc::clone(&pending_output_lines);
            move || {
                let stderr_sender = stderr_sender.lock().unwrap();
                while let Some(Ok(line)) = child_stderr.next() {
                    let barrier_token = line
                        .strip_prefix(runtime::EVCXR_STDERR_COMPLETE)
                        .and_then(|suffix| suffix.strip_prefix(' '));
                    let is_expected_barrier = barrier_token.is_some_and(|token| {
                        let mut expected = expected_stderr_barrier.lock().unwrap();
                    ...
                    if is_expected_barrier {
                        let token = barrier_token.unwrap();
                        let _ = stderr_barrier_sender.send(token.to_owned());
                        continue;
                    }
                    // Ignore errors, since it just means that the user of the library has dropped the receive end.
                    pending_output_lines.fetch_add(1, Ordering::SeqCst);
    ...
    pub(crate) fn restart(&mut self) -> Result<ChildProcess, Error> {
        ...
        ChildProcess::new_internal(
            ...
            self.stderr_barrier_sender.clone(),
            self.stderr_barrier_receiver.clone(),
            Arc::clone(&self.expected_stderr_barrier),
            Arc::clone(&self.pending_output_lines),
        )
    }

    pub(crate) fn prepare_stderr_barrier(&self, token: &str) {
        *self.expected_stderr_barrier.lock().unwrap() = Some(token.to_owned());
    }

    pub(crate) fn wait_for_stderr_barrier(&self, expected_token: &str) -> Result<(), Error> {
        loop {
            let token = self
                .stderr_barrier_receiver
                .recv()
                .map_err(|_| anyhow::anyhow!("Evaluation stderr barrier channel closed"))?;
