...
fn is_bayma_protected_environment(name: &str) -> bool {
    let name = name.to_ascii_uppercase();
    name == "PATH"
        || name == "CARGO_HOME"
        || name == "EVCXR_CONFIG_DIR"
        || name == "EVCXR_IGNORE_CWD_CONFIG"
        || name == "EVCXR_TMPDIR"
        || name == "RUSTC"
    ...
    fn create_commands() -> Vec<AvailableCommand> {
        vec![
            ...
            AvailableCommand::new(
                ...
                |ctx, state, args| {
                    if state.bayma_sealed() {
                        bail!("Bayma seals EVcxR startup configuration");
                    }
            ...
            AvailableCommand::new(
                ...
                |_ctx, state, args| {
                    if let Some(arg) = args {
                        if state.bayma_sealed() {
                            bail!("Bayma seals the packaged Rust toolchain");
                        }
            ...
            AvailableCommand::new(
                ":lockfile",
                "Resolve dependencies against a Cargo.lock (path), or stop (no argument)",
                |_ctx, state, args| {
                    let path = args.as_deref().map(|arg| match arg.strip_prefix("~/") {
                        Some(rest) => std::env::var_os("HOME")
                            .map_or_else(|| std::path::PathBuf::from(arg), |home| {
                                std::path::PathBuf::from(home).join(rest)
                            }),
                        None => std::path::PathBuf::from(arg),
                    });
                    state.set_lockfile(path.as_deref())?;
                    text_output(match state.lockfile() {
                        Some(path) => format!("Lockfile: {}", path.display()),
                        None => "Lockfile: none".to_owned(),
                    })
                },
            ),
            AvailableCommand::new(
            ...
            AvailableCommand::new(
                ...
                |_ctx, state, args| {
                    if state.bayma_sealed() {
                        bail!("Bayma seals compiler-wrapper selection");
                    }
            ...
            AvailableCommand::new(
                ...
                |_ctx, state, args| {
                    if let Some(arg) = args.as_ref() {
                        let mebibytes: u64 = arg.parse().map_err(|_| anyhow!("Invalid value"))?;
                        let bytes = mebibytes
                            .checked_mul(1024 * 1024)
                            .ok_or_else(|| anyhow!("Cache size is too large"))?;
                        if let Some(maximum) = state.bayma_max_cache_bytes()
                            && bytes > maximum
                        {
                            bail!("Bayma cache limit is {} MiB", maximum / 1024 / 1024);
                        }
                        state.set_cache_bytes(bytes);
            ...
            AvailableCommand::new(
                ...
                |_ctx, state, args| {
                    if let Some(linker) = args {
                        if state.bayma_sealed() {
                            bail!("Bayma seals linker selection");
                        }
            ...
            AvailableCommand::new(
                ...
                |_ctx, state, args| {
                    if let Some(backend) = args {
                        if state.bayma_sealed() {
                            bail!("Bayma seals codegen-backend selection");
                        }
                ...
                |_ctx, state, args| {
                    if let Some(arg) = args
                        && let Some((key, value)) = arg.split_once('=') {
                            if state.bayma_sealed() && is_bayma_protected_environment(key) {
                                bail!("Bayma seals build authority {key}");
                            }
            ...
            AvailableCommand::new(
                ...
                |_ctx, state, args| {
                    if let Some(arg) = args
                        && let Some((key, value)) = arg.split_once('=') {
                            if state.bayma_sealed() && is_bayma_protected_environment(key) {
                                bail!("Bayma seals runtime authority {key}");
                            }
