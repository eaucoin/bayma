...
impl Error {
    /// Returns the complete compiler diagnostic when rustc supplied one.
    ///
    /// `Display` intentionally remains EVcxR's concise user-facing message.
    /// Embedders need this form for startup and setup failures, where the
    /// rendered diagnostic contains linker stderr that is otherwise lost.
    pub fn diagnostic_text(&self) -> String {
        let Error::CompilationErrors(errors) = self else {
            return self.to_string();
        };
        let mut output = String::new();
        for error in errors {
            let rendered = error.rendered();
            let diagnostic = if rendered.trim().is_empty() {
                error.message()
            } else {
...
mod diagnostic_text_tests {
    ...
    fn compilation_error(message: &str, rendered: Option<&str>) -> CompilationError {
        let mut json = JsonValue::new_object();
        if let Some(rendered) = rendered {
            json["rendered"] = rendered.into();
        }
        CompilationError {
            message: message.to_owned(),
            json,
            code_origins: Vec::new(),
            spanned_messages: Vec::new(),
            spanned_helps: Vec::new(),
            level: "error".to_owned(),
        }
    }

    #[test]
    fn diagnostic_text_preserves_rendered_compiler_output() {
        let error = Error::CompilationErrors(vec![compilation_error(
            "linking failed",
            Some("linking failed\nlinker stderr\n"),
        )]);
        assert_eq!(error.diagnostic_text(), "linking failed\nlinker stderr");
        assert_eq!(error.to_string(), "linking failed");
    }

    #[test]
    fn diagnostic_text_falls_back_to_the_concise_message() {
        let error = Error::CompilationErrors(vec![compilation_error("bad cell", None)]);
        assert_eq!(error.diagnostic_text(), "bad cell");
        assert_eq!(
            Error::Message("setup failed".to_owned()).diagnostic_text(),
            "setup failed"
        );
    }
}
