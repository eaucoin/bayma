...
/// A lockfile chosen with `:lockfile`, read once so every later Cargo run
/// resolves against the same contents.
#[derive(Clone, Debug)]
...
pub(crate) struct Config {
    ...
    /// A lockfile the session resolves dependencies against, captured when it was chosen.
    lockfile: Option<Lockfile>,
    pub(crate) toolchain: String,
    cargo_path: PathBuf,
    pub(crate) rustc_path: PathBuf,
    bayma_linker_path: Option<PathBuf>,
    bayma_sealed: bool,
    bayma_max_cache_bytes: Option<u64>,
    cache_bytes: u64,
...
impl Config {
    pub fn new(tmpdir: PathBuf, subprocess_path: PathBuf) -> Result<Self> {
        ...
        let bayma_sealed =
            std::env::var_os("BAYMA_EVCXR_SEALED").as_deref() == Some(std::ffi::OsStr::new("1"));
        let bayma_max_cache_bytes = if bayma_sealed {
            Some(
                std::env::var("BAYMA_RUST_MAX_CACHE_BYTES")
                    .map_err(|_| anyhow!("BAYMA_RUST_MAX_CACHE_BYTES is required"))?
                    .parse::<u64>()
                    .map_err(|_| anyhow!("BAYMA_RUST_MAX_CACHE_BYTES must be a u64"))?,
            )
        } else {
            None
        };
        Ok(Config {
            ...
            rustc_path,
            bayma_linker_path: if cfg!(windows) {
                std::env::var_os("BAYMA_RUST_LINKER_BIN").map(PathBuf::from)
            } else {
                None
            },
            bayma_sealed,
            bayma_max_cache_bytes,
            core_extern,
            target,
    ...
    /// A Cargo command run in the session's crate. Cargo rewrites that crate's
    /// `Cargo.lock` to cover only its current dependencies, so a chosen lockfile
    /// is restored before every invocation; Cargo then keeps each locked version
    /// that still matches, yanked ones included, and resolves only the rest.
    pub(crate) fn cargo_command(&self, command_name: &str) -> Result<Command> {
        if let Some(lockfile) = &self.lockfile {
            std::fs::write(self.crate_dir().join("Cargo.lock"), &lockfile.contents).with_context(
                || format!("Failed to apply lockfile {}", lockfile.path.display()),
            )?;
        }
        ...
        if let Some(linker) = &self.bayma_linker_path {
            rustflags.push(format!("-Clinker={}", linker.display()));
        }
        ...
        command
            ...
            // The evaluation child needs the per-session checkpoint path at runtime, but Cargo,
            // build scripts, dependencies, and rustc do not own it. Keeping it out of the build
            // environment prevents both accidental compile-time capture and false cache
            // partitioning between otherwise identical sessions.
            .env_remove("BAYMA_RUST_CHECKPOINT_PATH")
            .env_remove("RUSTFLAGS")
            .env("CARGO_ENCODED_RUSTFLAGS", rustflags.join("\x1f"))
            .env(crate::module::CORE_EXTERN_ENV, &self.core_extern);
        if let Some(linker) = &self.bayma_linker_path {
            command.env(cargo_target_linker_environment(&self.target), linker);
        }
...
const SEND_TEXT_PLAIN_DEF: &str = stringify!(
    fn evcxr_send_text_plain(text: &str, evcxr_protocol_token: &str) {
        use std::io::Write;
        use std::io::{self};
        fn try_send_text(text: &str, evcxr_protocol_token: &str) -> io::Result<()> {
            let stdout = io::stdout();
            let mut output = stdout.lock();
            writeln!(
                output,
                "EVCXR_BEGIN_CONTENT {evcxr_protocol_token} text/plain"
            )?;
            output.write_all(text.as_bytes())?;
            writeln!(output, "\nEVCXR_END_CONTENT {evcxr_protocol_token}")?;
            Ok(())
        }
        if let Err(error) = try_send_text(text, evcxr_protocol_token) {
...
pub struct EvalContextOutputs {
    ...
    pub pending_output_lines: Arc<AtomicUsize>,
}
...
pub struct EvalCallbacks<'a> {
    ...
    pub execution_started: &'a dyn Fn(),
    pub execution_finished: &'a dyn Fn(),
}
...
fn default_execution_callback() {}

impl Default for EvalCallbacks<'_> {
    fn default() -> Self {
        EvalCallbacks {
            input_reader: &default_input_reader,
            execution_started: &default_execution_callback,
            execution_finished: &default_execution_callback,
        }
    }
}

struct ExecutionCallbackGuard<'a> {
    finished: &'a dyn Fn(),
}

impl Drop for ExecutionCallbackGuard<'_> {
    fn drop(&mut self) {
        (self.finished)();
    }
}
...
impl EvalContext {
    ...
    pub fn with_subprocess_command(
        ...
        let pending_output_lines = Arc::new(AtomicUsize::new(0));
        ...
        let mut context = EvalContext {
            ...
            child_process,
            stdout_sender,
            pending_output_lines: Arc::clone(&pending_output_lines),
            analyzer,
            initial_config,
        };
    ...
    fn run_and_capture_output(
        ...
        getrandom::fill(&mut token_bytes)
            .map_err(|error| anyhow!("Failed to create execution protocol identity: {error}"))?;
        let mut execution_token = String::with_capacity(token_bytes.len() * 2);
        for byte in token_bytes {
            write!(&mut execution_token, "{byte:02x}").unwrap();
        }
        ...
        .map_err(|error| anyhow!("Failed to encode execution request: {error}"))?;
        if state.config.bayma_sealed {
            let drain_started = Instant::now();
            while self.pending_output_lines.load(Ordering::SeqCst) != 0 {
                if drain_started.elapsed() >= Duration::from_secs(5) {
                    bail!("Output receivers did not drain before Rust execution");
                }
                std::thread::sleep(Duration::from_millis(1));
            }
        }
        self.child_process.prepare_stderr_barrier(&execution_token);
        self.child_process.send(&request)?;

        let execution_started = format!("{} {}", runtime::EVCXR_EXECUTION_STARTED, execution_token);
        loop {
            if self.child_process.recv_line()? == execution_started {
                break;
            }
        }
        (callbacks.execution_started)();
        let _execution_callback_guard = ExecutionCallbackGuard {
            finished: callbacks.execution_finished,
        };
        let run = serde_json::to_string(&("RUN", &execution_token))
            .map_err(|error| anyhow!("Failed to encode RUN acknowledgement: {error}"))?;
        self.child_process.send(&run)?;
        let execution_complete =
            format!("{} {}", runtime::EVCXR_EXECUTION_COMPLETE, execution_token);

        state.build_num += 1;

        let mut got_panic = false;
        let mut got_user_error = false;
        let mut lost_variables = Vec::new();
        let panic_notification = format!("{PANIC_NOTIFICATION} {execution_token}");
        let user_error = format!(
            "{} {}",
            evcxr_internal_runtime::USER_ERROR_OCCURRED,
            execution_token
        );
        let variable_changed_type = format!(
            "{}{}:",
            evcxr_internal_runtime::VARIABLE_CHANGED_TYPE,
            execution_token
        );
        let mime_output = format!("EVCXR_BEGIN_CONTENT {execution_token} ");
        let mime_end = format!("EVCXR_END_CONTENT {execution_token}");
        loop {
            let line = self.child_process.recv_line()?;
            if line == execution_complete {
                break;
            }
            if line == panic_notification {
                got_panic = true;
            } else if !state.config.bayma_sealed && line.starts_with(evcxr_input::GET_CMD) {
            ...
            } else if let Some(variable_name) = line.strip_prefix(&variable_changed_type) {
                lost_variables.push(variable_name.to_owned());
            } else if let Some(mime_type) = line.strip_prefix(&mime_output) {
                ...
                self.pending_output_lines.fetch_add(1, Ordering::SeqCst);
                if self.stdout_sender.send(line).is_err() {
                    self.pending_output_lines.fetch_sub(1, Ordering::SeqCst);
                }
            }
        }
        self.child_process
            .wait_for_stderr_barrier(&execution_token)?;
        if got_panic {
            output.did_panic = true;
            state
                .variable_states
                .retain(|_variable_name, variable_state| {
                    variable_state.move_state != VariableMoveState::New
        ...
        output.did_user_error = got_user_error;
...
pub struct EvalOutputs {
    ...
    /// Whether EVcxR caught a panic raised while evaluating this cell.
    pub did_panic: bool,
    /// Whether the question-mark operator returned early with a user error.
    pub did_user_error: bool,
}
...
impl EvalOutputs {
    ...
    pub fn merge(&mut self, mut other: EvalOutputs) {
        self.did_panic |= other.did_panic;
        self.did_user_error |= other.did_user_error;
...
impl ContextState {
    ...
    pub fn bayma_sealed(&self) -> bool {
        self.config.bayma_sealed
    }

    pub fn bayma_max_cache_bytes(&self) -> Option<u64> {
        self.config.bayma_max_cache_bytes
    }
    ...
    /// Resolve this session's dependencies against `path`'s `Cargo.lock`
    /// contents, or stop doing so with `None`.
    pub fn set_lockfile(&mut self, path: Option<&Path>) -> Result<()> {
        self.config.lockfile = match path {
            None => None,
            Some(path) => {
                ...
                std::str::from_utf8(&contents)
                    ...
                    .filter(|table| table.contains_key("package"))
                    .with_context(|| format!("{} is not a Cargo.lock", path.display()))?;
                Some(Lockfile {
    ...
    pub fn lockfile(&self) -> Option<&Path> {
        self.config.lockfile.as_ref().map(|lockfile| lockfile.path.as_path())
    }
    ...
    fn analysis_code(&self, user_code: CodeBlock) -> CodeBlock {
        let mut code = CodeBlock::new()
            ...
            .add_all(self.error_trait_code())
            .generated("macro_rules! __bayma_evcxr_protocol_identity { () => { \"analysis-protocol-token\" }; }")
    ...
    fn code_to_compile(
        ...
        if has_user_code {
            ...
            code = code
                ...
                .generated("mut x: *mut std::os::raw::c_void, _bayma_evcxr_protocol_identity_ptr: *const u8, _bayma_evcxr_protocol_identity_len: usize) -> *mut std::os::raw::c_void {x}");
        }
        code
    }
    ...
    fn error_trait_code(&self) -> CodeBlock {
    ...
    fn wrap_user_code(
        ...
        if self.allow_question_mark {
            code = code.add_all(self.error_trait_code());
        }
        ...
        if needs_variable_store {
            code = code
                .generated("mut evcxr_variable_store: *mut evcxr_internal_runtime::VariableStore, bayma_evcxr_protocol_identity_ptr: *const u8, bayma_evcxr_protocol_identity_len: usize)")
                .generated("  -> *mut evcxr_internal_runtime::VariableStore {")
                .generated("let __bayma_evcxr_protocol_identity_value = unsafe { std::str::from_utf8_unchecked(std::slice::from_raw_parts(bayma_evcxr_protocol_identity_ptr, bayma_evcxr_protocol_identity_len)) };")
                .generated("macro_rules! __bayma_evcxr_protocol_identity { () => { __bayma_evcxr_protocol_identity_value }; }")
            ...
            code = code
                .generated("evcxr_variable_store: *mut u8, bayma_evcxr_protocol_identity_ptr: *const u8, bayma_evcxr_protocol_identity_len: usize) -> *mut u8 {")
                .generated("let __bayma_evcxr_protocol_identity_value = unsafe { std::str::from_utf8_unchecked(std::slice::from_raw_parts(bayma_evcxr_protocol_identity_ptr, bayma_evcxr_protocol_identity_len)) };")
                .generated("macro_rules! __bayma_evcxr_protocol_identity { () => { __bayma_evcxr_protocol_identity_value }; }");
        }
        if self.async_mode {
            let mut async_code = CodeBlock::new().generated(stringify!(
                let mut mutex = evcxr_variable_store.lazy_arc("evcxr_tokio_runtime",
                    || std::sync::Mutex::new(tokio::runtime::Runtime::new().unwrap())
                );
            ));
            if self.allow_question_mark {
                async_code = async_code.generated("let evcxr_question_result = ");
            }
            user_code = async_code
                .generated(stringify!(
            ...
            if self.allow_question_mark {
                user_code = user_code.generated("if evcxr_question_result.is_err() { println!(\"{} {}\", evcxr_internal_runtime::USER_ERROR_OCCURRED, __bayma_evcxr_protocol_identity!()); }");
            }
        } else if self.allow_question_mark {
            user_code = CodeBlock::new()
                .generated("let evcxr_question_result = (|| -> std::result::Result<(), EvcxrUserCodeError> {")
                .add_all(user_code)
                .generated("Ok(())})();")
                .generated("if evcxr_question_result.is_err() { println!(\"{} {}\", evcxr_internal_runtime::USER_ERROR_OCCURRED, __bayma_evcxr_protocol_identity!()); }");
        }
        if compilation_mode == CompilationMode::RunAndCatchPanics {
            if needs_variable_store {
                code = code
                    ...
                    .generated("  Err(_) => {")
                    .generated(format!(
                        "    println!(\"{{}} {{}}\", \"{PANIC_NOTIFICATION}\", __bayma_evcxr_protocol_identity!());"
                    ))
                    .generated("}}");
            } else {
                code = code
    ...
    fn check_variable_statements(&self) -> CodeBlock {
        ...
        for (var_name, var_state) in &self.stored_variable_states {
            statements = statements.generated(format!(
                "vars_ok &= evcxr_variable_store.check_variable::<{}>(stringify!({var_name}), __bayma_evcxr_protocol_identity!());",
                var_state.type_name
            ));
        }
                    ...
                    if self.config.display_final_expression {
                        code_out = code_out.code_with_fallback(
                            ...
                            if self.config.display_types {
                                ...
                                .generated(format!(
                                    "); evcxr_send_text_plain(&format!(\": {{}} = {}\", evcxr_get_type_name(r), r), __bayma_evcxr_protocol_identity!()); }};",
                                    self.config.output_format,
                                ))
                            } else {
                                CodeBlock::new()
                                ...
                                .with_segment(segment)
                                .generated(")), __bayma_evcxr_protocol_identity!());")
                                },
                            );
                    } else {
                        code_out = code_out
...
fn default_cargo_path() -> Result<PathBuf> {
    if let Some(path) = std::env::var_os("BAYMA_CARGO_BIN") {
...
fn default_rustc_path() -> Result<PathBuf> {
    if let Some(path) = std::env::var_os("BAYMA_RUSTC_BIN") {
        return Ok(PathBuf::from(path));
    }
...
fn cargo_target_linker_environment(target: &str) -> String {
    format!(
...
mod tests {
    ...
    #[test]
    fn test_cargo_target_linker_environment() {
        assert_eq!(
            cargo_target_linker_environment("aarch64-pc-windows-gnullvm"),
