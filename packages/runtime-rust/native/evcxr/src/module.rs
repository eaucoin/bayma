...
pub(crate) fn wrap_rustc_helper() -> Result<i32> {
    ...
    // Compilation-cache I/O and identity inspection are opportunistic. A cache fault must never
    // replace rustc's authority or turn an otherwise valid user dependency into a failed build.
    let cache_result = cache::access_cache(&command).unwrap_or(CacheResult::NonCache);
    ...
    if output.status.code() == Some(0) {
        ...
        if let CacheResult::Miss(cache_miss) = cache_result {
            let _ = cache_miss.update_cache(&artifacts);
        }
    }
