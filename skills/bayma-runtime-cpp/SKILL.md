...
Create sessions with `runtime: "cpp"`; the selected runtime remains fixed for the session lifetime. Do not compile throwaway programs or one-off C++ executions instead of using bayma.
...
Live definitions, variables, headers, and libraries persist across execs and client reconnects while the interpreter remains up. A crash, uncaught exception, exit, or interrupt replaces the interpreter; only the last committed checkpoint carries over. Durable exec resources are the authority on whether submitted work compiled and ran: `bayma:///session/{sessionId}/exec/{execId}` for status and code, and `/messages` for ordered output and errors.
...
- `__cplusplus`, `__clang_version__`, and `_LIBCPP_VERSION` or `__GLIBCXX__` for the language and standard library in use;
- `std::filesystem::current_path()` and relevant environment variables;
...
Source and type inspection is useful for two things: understanding a repository or codebase, and understanding libraries and their APIs, whether a project's dependencies, the standard library, or any other library. For C++, the tools below do both within the session, so translation units, cursors, and types stay live in the REPL across cells, ready to be queried, compared, and built on, rather than reconstructed from a shell command's output each time.
