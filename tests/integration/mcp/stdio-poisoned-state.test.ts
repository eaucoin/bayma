...
const poisonedStateStartupBudgetMs = 5_000;
const poisonedStateTestTimeoutMs = 10_000;

test(
  "mcp-stdio boots with poisoned persisted state without blocking",
  async () => {
    await withTempDir(async (dir) => {
      ...
      const client = await McpStdioClient.connect({ stateDir: dir });
      try {
        expect(Date.now() - startedAtMs).toBeLessThan(
          poisonedStateStartupBudgetMs,
        );
  ...
  { timeout: poisonedStateTestTimeoutMs },
);
